...
//<!***************************************************************************
class ROCKETMQCLIENT_API DefaultMQPushConsumer : public MQConsumer/* 虚基类 */ {
 public:
 ...
 private:
  ...
  // boost::thread 类提供了一个静态方法 hardware_concurrency() ，
  // 它能够返回基于CPU数目或者CPU内核数目的刻在同时在物理机器上运行的线程数。
  // 在常用的双核机器上调用这个方法，返回值为2。这样的话就可以确定在一个多核程序可以同时运行的理论最大线程数。
